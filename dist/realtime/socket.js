...
const broadcastedPresenceByUser = new Map();
const presenceUpdateQueue = new Map();
function addAllCurrentSockets(info, userId) {
...
function addParticipant(info, userId, socketId) {
    const existing = info.participantsByUser.get(userId) ?? new Set();
    existing.add(socketId);
    info.participantsByUser.set(userId, existing);
}
function removeParticipant(info, userId, socketId) {
...
function listParticipants(info) {
    return Array.from(info.participantsByUser.keys());
}
function isUserInAnyCall(userId) {
    for (const info of activeGroupCalls.values()) {
        if (info.participantsByUser.has(userId))
            return true;
    }
...
function computeBroadcastPresence(userId, base) {
    if (base === "OFFLINE")
        return "OFFLINE";
    if (isUserInAnyCall(userId))
        return "IN_CALL";
    return base;
}
function emitEffectivePresence(io, userId, baseOverride) {
...
function initSocket(server) {
    ...
    io.on("connection", (socket) => {
        ...
        try {
            ...
            for (const info of activeGroupCalls.values()) {
                for (const uid of info.participantsByUser.keys())
                    inCallUsers.add(uid);
            }
            for (const info of activeDirectCalls.values()) {
                for (const uid of info.participantsByUser.keys())
        ...
        socket.on("call:invite", async ({ conversationId, video }) => {
            ...
            if (isGroup) {
                const callInfo = activeGroupCalls.get(conversationId);
                if (!callInfo) {
                    const info = { startedAt, participantsByUser: new Map() };
        ...
        socket.on("call:accept", async ({ conversationId, video }) => {
            ...
            if (!isGroup) {
                ...
                const info = activeDirectCalls.get(conversationId) ?? { startedAt, participantsByUser: new Map() };
                ...
                if (st?.inviterId)
                    emitEffectivePresence(io, st.inviterId);
                emitEffectivePresence(io, userId);
            }
        ...
        socket.on("call:room:join", async ({ conversationId, video }) => {
            ...
            if (!callInfo) {
                ...
                callInfo = { startedAt: callState.get(conversationId)?.startedAt ?? Date.now(), participantsByUser: new Map() };
