...
## Вертикальный таймлайн (рельса)

Слева от плиток в таймлайне — ось со «станциями»: круглая миниатюра дня, дата
и счётчик. Пройденный отрезок закрашен, клик — прыжок к дате; если дни не
помещаются по высоте, станции сами укрупняются до месяцев и лет.

Служебные маршруты (все под сессией Cloud):

- `GET /api/cloud/files/timeline?spaceId&tz&kind&q&view` — счётчик и
  представительский файл на каждый день съёмки (день считается в локальном
  поясе клиента через `tz`, минуты к востоку от UTC);
- `GET /api/cloud/files/ids?spaceId&view&…` — id всего среза для «Выбрать все»
  (кап 20 000, флаг `truncated`);
- `POST /api/cloud/files/zip/prepare` → `GET /api/cloud/files/zip?token=…` —
  именной талон на архив по выделению в Redis (TTL 5 минут): списки в тысячи
  id не лезут в строку запроса.

## Места съёмки

Вкладка «Места» — сестра таймлайна: та же рельса и та же галерея, только
шкала географическая. Страна → город → район; глубже не идём сознательно,
у справочника нет городских кварталов, сразу за ними начинаются улицы.

Геокодирование ЦЕЛИКОМ офлайн, без обращений к чужим сервисам: координаты
личных снимков наружу не уходят. Работает частью обычной обработки медиа —
`applyPlace` в `src/cloud/jobs/mediaWorker.ts` вызывается сразу после чтения
EXIF, так что новые загрузки получают место автоматически.

Справочник — GeoNames (CC BY 4.0, https://www.geonames.org/), кладётся в
`CLOUD_STORAGE_ROOT/geo`:

```
npm run cloud:geo-fetch        # cities500 (~13 МБ) + русские названия (~190 МБ разово)
npm run cloud:geo-backfill     # проставить место уже загруженным файлам
npm run cloud:geo-backfill -- --all   # пересчитать заново после обновления справочника
```

Как выбирается место: у каждого населённого пункта есть радиус влияния по
населению, поэтому крупный город забирает снимки своих окраин, а деревня —
только собственные. Район подписывается ТОЛЬКО когда ближайший пункт честно
ближе города: иначе на весь центр вешался бы соседний посёлок. Русские
названия берутся из справочника с языковыми метками — без него выходило
«Парис» вместо «Парижа».

Служебный маршрут: `GET /api/cloud/files/places?spaceId=…` — иерархия мест со
счётчиками, представительским снимком и числом снимков без геометки.
...
## Домены и монтирование

Cloud собирается в один бандл и обслуживает два расклада одновременно:

| Адрес | Режим | Пути |
|---|---|---|
| `eblusha.cloud` | домен отдан Cloud целиком | `/`, `/space/:id`, `/s/:publicId` |
| `cloud.eblusha.org` | то же | то же |
| `eblusha.org/cloud` | рядом с мессенджером | `/cloud`, `/cloud/space/:id` |

Режим определяет `frontend/src/cloud/basePath.ts` по хосту (список
`CLOUD_ONLY_HOSTS`, переопределяется мета-тегом `eb-cloud-base` без пересборки).
По pathname определять нельзя: на `eblusha.org` корень принадлежит чату, и
cloud-маршруты обязаны сидеть под `/cloud` независимо от точки входа.

На выделенном домене маршруты мессенджера **не монтируются вообще** — корень
занимает Cloud. Старые пути `/cloud/...` там отдают 301 на новые; фрагмент
(`#t=<секрет>` у приглашений и share-ссылок) браузер переносит сам.

Генерация ссылок на сервере управляется `CLOUD_PUBLIC_PATH_PREFIX`:
пусто — `https://eblusha.cloud/s/<id>`, `/cloud` — `https://eblusha.org/cloud/s/<id>`.

Точка возврата после входа тоже зависит от режима (`/callback` либо
`/cloud/callback`), и обе разрешены в allowlist'е `isSafeRedirect`.
