...
1. DNS: `cloud` → тот же тоннель, что и `eblusha.org` (CNAME на `<tunnel-id>.cfargotunnel.com`).
2. Zero Trust → Networks → Tunnels → тоннель `EBLUSHA` → Public Hostnames →
   Add: hostname `cloud.eblusha.org`, **origin service ровно такой же, как у
   `eblusha.org`** (тот же адрес и порт локального nginx).

Больше ничего: nginx уже слушает `server_name cloud.eblusha.org` и на 80, и на
443, так что вариант origin service роли не играет.
...
## Cloudflare CDN для файлов

Короткий ответ: **как ускоритель раздачи файлов — не работает, и включать его
ради этого не нужно.** Причины по порядку значимости.

**1. Приватные файлы не кэшируются, и это правильно.** Всё, что отдаёт Cloud,
идёт с `Cache-Control: private` и требует куку либо share-сессию. Cloudflare
такое не кэширует (и не должен: закэшированное на edge приватное фото — это
утечка). Значит CDN здесь не экономит ни байта, а добавляет лишний хоп.

**2. ToS 2.8 self-serve планов.** Раздача существенного объёма видео и крупных
файлов через проксируемый Cloudflare, минуя Stream/R2/Images, формально
запрещена. Домашнее облако с гигабайтами MOV — ровно этот случай. Риск не
теоретический: за это отключают проксирование.

**3. Лимит тела запроса.** На Free это 100 МБ на запрос. Загрузки это переживают
только потому, что tus режет файл на куски по 16 МБ (`CHUNK_SIZE` в
`frontend/src/cloud/uploads/manager.ts`). **Если поднимать размер чанка — не
выше ~90 МБ**, иначе загрузка через Cloudflare начнёт падать на 413.

**Что Cloudflare реально даёт и уже даёт сейчас:** TLS, сокрытие IP сервера,
защита от сканеров и DDoS, и кэш статики SPA. Последнее включено осознанно:
`/assets/` отдаётся с `public, max-age=31536000, immutable` (файлы хешированные),
и вот их edge кэширует прекрасно — это единственный контент Cloud, которому CDN
объективно помогает.

**Если хочется ускорить именно медиа для друзей в США и России**, работающих
вариантов три, и все они — смена модели, а не галочка:

* отдавать **публичные share-ссылки** с публичным кэшем. Технически возможно
  (секрет уже в capability-URL), но это переносит контроль доступа на edge и
  ломает мгновенный revoke. Не рекомендую;
* Cloudflare **R2 + Stream** как storage backend — противоречит требованию
  «оригиналы лежат на своём диске в Румынии»;
* просто уменьшить вес того, что реально гоняется: превью уже 512px WebP и
  иммутабельны на 30 дней в браузере, а видео отдаётся оригиналом там, где он
  играбелен. Основной выигрыш здесь уже получен.

Отдельно: `proxy_request_buffering off` в nginx-конфиге поддомена оставлен
сознательно — без него чанки tus оседали бы во временном файле nginx целиком,
и прогресс загрузки врал бы пользователю.
