...
### 3. (Опционально) Установите API_BASE_URL

Для миграционного скрипта можно установить `API_BASE_URL`, чтобы он использовал прокси URL вместо прямых ссылок на S3:

```bash
export API_BASE_URL=https://ru.eblusha.org
```

Если не указано, будет использован origin из `STORAGE_PUBLIC_BASE_URL` или `http://localhost:3000` по умолчанию.
