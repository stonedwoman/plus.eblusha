...
final class MessageSwipeState: ObservableObject {
    ...
    /// Рамка полосы реакций в координатах ячейки: по ней лента понимает, что палец лёг
    /// на чип, и не считает второй тап по нему двойным тапом «быстрая реакция».
    var reactionsFrame: CGRect = .zero
...
struct ChatView: View {
    ...
    var body: some View {
        ...
        // Полотно беседы — серая поверхность, как панель сообщений в вебе
        // (.messages-pane, background surface-200). Пузыри темнее фона (bubbleIn),
        // поэтому на почти чёрном paper они сливались и экран выглядел плоским.
